...
def train():
    ...
    model = bspl.PixelMorphNet(config).cuda()
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(dataset), 1):
        ...
        save_file_dir = os.path.join(args.model_dir, "tongji_morph_train_fold{}_5e-2.pth".format(fold))
    ...
    # 在所有fold结束后，保存最佳模型参数
    save_file_dir = os.path.join(args.model_dir, "morph_best_1e-6.pth")
