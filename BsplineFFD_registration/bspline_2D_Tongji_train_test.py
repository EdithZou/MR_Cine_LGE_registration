...
import model.TransMorph_bspl_ACDC as bspl
...
parser.add_argument("--csv_train_dir", type=str, help="csv train", dest="csv_train", default="/home/fguo24/projects/def-ouriadov/fguo24/GuoLab_students/tzou/Tongji_CineLGE_2D/Tongji_CineLGE_2D_fguo24.csv")
parser.add_argument("--csv_test_dir", type=str, help="csv test", dest="csv_test", default="/home/fguo24/projects/def-ouriadov/fguo24/GuoLab_students/tzou/Tongji_CineLGE_2D/Tongji_CineLGE_2D_test.csv")
...
def compute_Jacobian(flow):
    ...
    参数：
    flow (torch.Tensor): 表示形变场的张量，形状为 (B, 2, H, W)，
                         其中 B 是批大小，2 表示形变场的两个分量，
                         H 和 W 是形变场的高度和宽度。

    返回：
    torch.Tensor: 形状为 (B, H-1, W-1) 的张量，表示雅各布行列式。
    """
    # 计算偏导数
    dF1dx = flow[:, 0, 1:, :-1] - flow[:, 0, :-1, :-1]
    dF1dy = flow[:, 0, :-1, 1:] - flow[:, 0, :-1, :-1]
    dF2dx = flow[:, 1, 1:, :-1] - flow[:, 1, :-1, :-1]
    dF2dy = flow[:, 1, :-1, 1:] - flow[:, 1, :-1, :-1]
...
class Jacobian:
    ...
    def count_minus_ratio(self):
        ...
        float: 非正值的比例。
        """
        size = self.determinant.numel()
        x = torch.where(self.determinant <= 0, torch.tensor(1.0).cuda(), torch.tensor(0.0).cuda())
...
def train():
    ...
    print("iter,train_loss,sim_loss,reg_loss,valid_loss,sim_loss,reg_loss", file=file_log)
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(dataset), 1):
        ...
        #print(len(dataset),len(train_dataset), len(valid_dataset), len(test_dataset))
        # 创建对应的数据加载器
        ...
        for _iter in range(1, args.n_iter + 1):
            ...
            for batch in dataloader:
                ...
                #img_moving = batch['img_moving'].cuda()
                #img_fixed = batch['img_fixed'].cuda()
                img_moving = img_moving.permute(0,3,1,2)
                img_fixed = img_fixed.permute(0,3,1,2)
                #print(img_moving.shape, img_fixed.shape)
                # 输入网络
                #img_warped, flow = model(img_moving, img_fixed, mode='train')
                img_warped, flow = model(img_moving, img_fixed, mode='train')
                # 计算loss
            ...
            with torch.no_grad():
                ...
                for batch in dataloader_valid:
                    ...
                    img_moving = img_moving.permute(0,3,1,2)
                    img_fixed = img_fixed.permute(0,3,1,2)                
                    #img_warped, flow = model(img_moving, img_fixed, mode='valid')
                    img_warped, flow = model(img_moving, img_fixed, mode='valid')
                    # 计算dice
        ...
        save_file_dir = os.path.join(args.model_dir, "tongji_train_bspl_fold{}_5e-2.pth".format(fold))
        ...
        # 创建配准网络 unet+stn
        print("----Build registration network----")
        model_test = bspl.CubicBSplineNet(config).cuda()
        #stn = SpatialTransformer([128,128]).cuda()
        #dice_fn = Dice().cuda()
        model_test.load_state_dict(torch.load(save_file_dir))
        model_test.eval()
        # 开始测试
        print("----Start testing----")
        # 计时
        ...
        for batch in dataloader_test:
            img_moving, subject_mov, slicing_mov = crop_image_128_test(batch['img_moving'])
            img_moving = img_moving.cuda()
            img_fixed, subject_fix, slicing_fix = crop_image_128_test(batch['img_fixed'])
            img_fixed = img_fixed.cuda()        
            ...
            # 输入网络计时
            img_warped, flow = model_test(img_moving, img_fixed, mode='test')
            ...
            nib.save(img, os.path.join(subject_dir,f"fold{fold}_warped_{slicing_mov}.nii.gz"))
            img = nib.Nifti1Image(img_fixed[0,0,:,:].cpu().detach().numpy(), None)
            nib.save(img, os.path.join(subject_dir,f"fold{fold}_fixed_{slicing_mov}.nii.gz"))
            img = nib.Nifti1Image(img_moving[0,0,:,:].cpu().detach().numpy(), None)
            nib.save(img, os.path.join(subject_dir,f"fold{fold}_moving_{slicing_mov}.nii.gz"))
            flow = flow.permute(0,2,3,1)
            flow = flow[0]
            DVF = nib.Nifti1Image(flow[:,:,:].cpu().detach().numpy()[:,:,None,:][:,:,:,None,:], None)
            nib.save(DVF, os.path.join(subject_dir, f"fold{fold}_flow_{slicing_mov}.nii.gz"))


        print("time_used = %f" % np.sum(time_list))

        # 保存结果
        with open(os.path.join(args.result_dir, f"fold{fold}_result.csv"), "w") as f:
            writer = csv.writer(f)
            header = ["time"]
    ...
    # 在所有fold结束后，保存最佳模型参数
    save_file_dir = os.path.join(args.model_dir, "bspl_best_model_5e-2.pth")
...
def test():
    ...
    for batch in dataloader:
        ...
        start_time = time.time()
        # 输入网络计时
        img_warped, flow = model(img_moving, img_fixed, mode='test')
        label_warped = stn(label_moving, flow)
        time_list.append([float(time.time() - start_time)])
        # 计算dice
        if args.dice:
        ...
        label = nib.Nifti1Image(label_fixed[0, :, :, :].cpu().detach().type(torch.int8).numpy(), None)
        nib.save(label, os.path.join(subject_dir, f"label_fixed_{slicing_mov}.nii.gz"))
        label = nib.Nifti1Image(label_moving[0, :, :, :].cpu().detach().type(torch.int8).numpy(), None)
        nib.save(label, os.path.join(subject_dir, f"label_moving_{slicing_mov}.nii.gz"))        
        label = nib.Nifti1Image(label_warped[0, :, :, :].cpu().detach().type(torch.int8).numpy(), None)
    ...
    with open(os.path.join(args.result_dir, "result.csv"), "w") as f:
        ...
        if args.dice:
            header.append("aver_dice_pre")
            header.append("aver_dice_done")
