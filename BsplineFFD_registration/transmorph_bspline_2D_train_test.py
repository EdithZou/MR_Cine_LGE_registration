...
# 文件路径
parser.add_argument("--train_dir", type=str, help="data folder with training vols", dest="train_dir", default="/home/fguo/projects/def-gawright/fguo/GuoLab_students/tzou/Xiehe2D_resi")
...
def train():
    ...
    print("iter,train_loss,sim_loss,grad_loss,reg_loss,valid_loss,sim_loss,grad_loss,reg_loss", file=file_log)

    # 创建配准网络 unet+stn
    print("----Build registration network----")
    config = CONFIGS_BSP['BSplineParam']
    #model = bspl.CubicBSplineNet(config).cuda()
    model = bspl.MorphNet(config).cuda()
    #loss_fn = LossFn.MILossGaussian()
    # 模型参数个数
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        for _step in range(steps):
            ...
            label_warped = warp(label_moving, disp)
            
            # 计算loss
            sim_loss = sim_loss_fn(img_warped, img_fixed)
            grad_loss = grad_loss_fn(flow)
            reg_loss = l2reg_loss(flow)
            dice_loss = compute_Dice(label_fixed, label_warped)
            loss = args.alpha * grad_loss + reg_loss + sim_loss - dice_loss
        ...
        with torch.no_grad():
            ...
            for _num in range(len(valid_list)):
                ...
                # 计算loss
                sim_loss = sim_loss_fn(img_warped, img_fixed)
                grad_loss = grad_loss_fn(flow) # 加上反向形变场损失
                reg_loss = l2reg_loss(flow)
                loss = args.alpha * grad_loss + reg_loss + sim_loss - dice_loss
        ...
        if loss_valid <= best_valid_loss:
            ...
            save_file_dir = os.path.join(args.model_dir, "morph_2000_nccdice.pth")
...
def test():
    ...
    for _form in range(2):
        ...
        for _num in range(len(test_list)):
            ...
            if len(label_list) > 0:
                file_path_label = label_list[0]
                #print('test label: ', file_path_label)
                data_label = torch.from_numpy(nib.load(file_path_label).get_fdata()).type(torch.int8)
