...
def train():
    ...
    print("iter,train_loss,sim_loss,grad_loss,reg_loss,dice_train,valid_loss,sim_loss,grad_loss,reg_loss,dice_valid", file=file_log)

    # 创建配准网络 unet+stn
    print("----Build registration network----")
    config = CONFIGS_PM['PixelmorphParam']
    model = morphbspl.BsplineNet(config).cuda()
    stn = morphbspl.SpatialTransformer([128,128]).cuda()
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        for _step in range(steps):
            ...
            loss = args.alpha * reg_loss + sim_loss - dice_loss
        ...
        print("epoch: %d  train_loss: %f  sim_loss: %f  reg_loss: %f train_dice: %f" % (_iter, loss_train, sim_loss_train, reg_loss_train, dice_train), flush=True)
        ...
        print("%d,%f,%f,%f,%f,%f,%f,%f,%f" % (_iter, loss_train, sim_loss_train, reg_loss_train, dice_train, loss_valid, sim_loss_valid, reg_loss_valid, dice_valid), file=file_log)
        ...
        if loss_valid <= best_valid_loss:
            ...
            save_file_dir = os.path.join(args.model_dir, "pixvm_train_2.pth")
