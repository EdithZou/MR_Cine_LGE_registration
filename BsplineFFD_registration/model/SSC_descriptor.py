...
def imshift(im1, x, y, pad=0):
    ...
    im1shift[..., y1:y2 + 1, x1:x2 + 1] = im1shift[..., y1s:y2s + 1, x1s:x2s + 1]
    return im1shift.to(device)


def SSC_descriptor2D(I, r=0, sigma=0.5):
    """
    Calculation of SSC (modality independent neighbourhood descriptor)
    :param I:
    :param r:
    :param sigma:
    :return:
    ...
    Dp = torch.zeros((len(xs0), *I.shape), device=I.device)
    shift = torch.zeros((len(xs0), *I.shape), device=I.device)
    for i in range(len(xs0)):
        shift[i,...] = imshift(I, xs0[i], ys0[i])
    Dp[0, ...] = gaussian_filter_torch(((shift[0,...] - shift[2,...]) ** 2), sigma=sigma)
    Dp[1, ...] = gaussian_filter_torch(((shift[0,...] - shift[3,...]) ** 2), sigma=sigma)
    Dp[2, ...] = gaussian_filter_torch(((shift[1,...] - shift[2,...]) ** 2), sigma=sigma)
    Dp[3, ...] = gaussian_filter_torch(((shift[1,...] - shift[3,...]) ** 2), sigma=sigma)
...
def compute_ssc(img, r=0, sigma=0.5):
    # 得到[4,C,H,W]的特征层
    mind = SSC_descriptor2D(img, r=r, sigma=sigma)
    return mind
