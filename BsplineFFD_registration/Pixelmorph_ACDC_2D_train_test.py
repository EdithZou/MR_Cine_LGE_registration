...
from model.Voxelmorph_bspl import CONFIGS as CONFIGS_PM
import model.Voxelmorph_bspl as morphbspl
...
def compute_nmi_loss(labels_true, labels_pred):
    # Flatten the inputs
    labels_true_flat = labels_true.reshape(labels_true.shape[0], -1)
    labels_pred_flat = labels_pred.reshape(labels_pred.shape[0], -1)
    
    # Convert multi-label pixel values to integer labels
    labels_true_flat = np.argmax(labels_true_flat, axis=1)
    labels_pred_flat = np.argmax(labels_pred_flat, axis=1)
    
    # Compute NMI loss
    nmi_loss = nmi_score(labels_true_flat, labels_pred_flat)
...
def train():
    ...
    config = CONFIGS_PM['PixelmorphParam']
    model = morphbspl.BsplineNet(config).cuda() 
    stn = SpatialTransformer([256,256]).cuda()
    ...
    valid_list = np.arange(60, 93)             

    dataset_train_img = torch.zeros([2, 60, 256, 256], dtype=torch.float32)
    dataset_train_label = torch.zeros([2, 60, 256, 256], dtype=torch.int8)
    dataset_valid_img = torch.zeros([2, 33, 256, 256], dtype=torch.float32)
    dataset_valid_label = torch.zeros([2, 33, 256, 256], dtype=torch.int8)
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        for _step in range(steps):
            ...
            img_fixed = torch.zeros([args.batch_size, 1, 256, 256], dtype=torch.float32)
            img_moving = torch.zeros([args.batch_size, 1, 256, 256], dtype=torch.float32)
            ...
            # 输入网络
            img_warped, flow, scaled_flow, scaled_warped = model(img_moving, img_fixed)
            
            # 计算loss
            sim_loss = sim_loss_fn(img_warped, img_fixed)
            reg_loss = bend_fn(flow)
            smth_loss = grad_loss_fn(scaled_flow)
        ...
        with torch.no_grad():
            model.eval()
            img_fixed = torch.zeros([1, 1, 256, 256], dtype=torch.float32)
            img_moving = torch.zeros([1, 1, 256, 256], dtype=torch.float32)
            label_fixed = torch.zeros([1, 1, 256, 256], dtype=torch.float32)
            label_moving = torch.zeros([1, 1, 256, 256], dtype=torch.float32)
            ...
            for _num in range(len(valid_list)):
                ...
                dice_loss = dice_fn(label_warped, label_fixed, labelorders=[1,2,3])
        ...
        '''
        print("epoch: %d  train_loss: %f  sim_loss: %f  reg_loss: %f" % (_iter, loss_train, sim_loss_train, reg_loss_train), flush=True)
        print("epoch: %d  valid_loss: %f  sim_loss: %f  reg_loss: %f valid_dice: %f" % (_iter, loss_valid, sim_loss_valid, reg_loss_valid, dice_valid), flush=True)
        print("%d,%f,%f,%f,%f,%f,%f,%f" % (_iter, loss_train, sim_loss_train, reg_loss_train,loss_valid, sim_loss_valid, reg_loss_valid, dice_valid), file=file_log)
        
        '''

        '''
        print("epoch: %d  train_loss: %f  sim_loss: %f  reg_loss: %f" % (_iter, loss_train, sim_loss_train, reg_loss_train), flush=True)
        print("epoch: %d  valid_loss: %f  sim_loss: %f  reg_loss: %f" % (_iter, loss_valid, sim_loss_valid, reg_loss_valid), flush=True)
        print("%d,%f,%f,%f,%f,%f,%f" % (_iter, loss_train, sim_loss_train, reg_loss_train, loss_valid, sim_loss_valid, reg_loss_valid), file=file_log)
        '''
        # 计时
        if _iter % 10 == 0:
        ...
        if loss_valid <= best_valid_loss:
            ...
            save_file_dir = os.path.join(args.model_dir, "pixmorph_ACDC_train.pth")
...
def test():
    ...
    # 测试序列
    test_list = np.arange(80, 93)

    # 读取图像数据
    dataset_img = torch.zeros([2, 13, 256, 256], dtype=torch.float32)
    dataset_label = torch.zeros([2, 13, 256, 256], dtype=torch.int8)
    subject_forms = ["ED", "ES"]
    ...
    for _num in range(len(test_list)):
        ...
        img_warped, flow, disp = model(img_moving, img_fixed)
        time_list.append([float(time.time() - start_time)])

        label_warped = stn(label_moving, disp)
        
        # 计算dice
        if args.dice:
            dice_list.append([dice_fn(label_fixed, label_moving, labelorders =[1,2,3]), dice_fn(label_fixed, label_warped, labelorders =[1,2,3])])
        # 计算psnr
        if args.psnr:
        ...
        dvf_for_mov2fix = flow.unsqueeze(0)
        dvf_for_mov2fix = dvf_for_mov2fix[0]        
        DVF = nib.Nifti1Image(dvf_for_mov2fix[:, :, :].permute([1,2,0]).cpu().detach().numpy()[:,:,None,:][:,:,:,None,:], None) # 变为三维网格，以便使用ITK-snap的grid格式
