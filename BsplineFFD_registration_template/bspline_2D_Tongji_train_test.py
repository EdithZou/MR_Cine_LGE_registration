...
def train():
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(dataset), 1):
        ...
        save_file_dir = os.path.join(args.model_dir, "tongji_temp_fold{}.pth".format(fold))
    ...
    # 在所有fold结束后，保存最佳模型参数
    save_file_dir = os.path.join(args.model_dir, "temp_model.pth")
...
def test():
    ...
    for batch in dataloader:
        ...
        flow = flow[0]
        print('save flow shape:', flow.shape)
