...
def train():
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        if loss_valid <= best_valid_loss:
            ...
            save_file_dir = os.path.join(args.model_dir, "fd_bspline_800_train.pth")
...
def test():
    ...
    for _num in range(len(test_list)):
        ...
        # 计算dice
        if args.dice:
            dice_list.append([dice_fn(label_fixed, label_moving, labelorders=[1,2,3]).item(), dice_fn(label_fixed, label_warped, labelorders=[1,2,3]).item()])
        # 计算psnr
        if args.psnr:
