...
class batch_SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):
        warped = torch.zeros([len(flow[0]), 2, 1, 128, 128], dtype=torch.float32)# batchsize, pair_num, channel, H,W
        device = src.device
        for i in range(len(flow[0])):
            img_input = src[i,:,:,:].unsqueeze(0).permute(1,0,2,3)
            disp = flow[i,:,:,:,:].to(device)
            new_locs = self.grid + disp
            shape = disp.shape[2:]
            # Need to normalize grid values to [-1, 1] for resampler
            for j in range(len(shape)):
                new_locs[:, j, ...] = 2 * (new_locs[:, j, ...] / (shape[j] - 1) - 0.5)
            if len(shape) == 2:
                new_locs = new_locs.permute(0, 2, 3, 1)
                new_locs = new_locs[..., [1, 0]]
            elif len(shape) == 3:
                new_locs = new_locs.permute(0, 2, 3, 4, 1)
                new_locs = new_locs[..., [2, 1, 0]]
            #print('warped i ', warped[i,:,:,:,:].shape)
            #print('warped', warped.shape)
            warped[i,:,:,:,:] = F.grid_sample(img_input, new_locs, mode=self.mode)
            #print('batch num', i, 'warped', warped[i,:,:,:,:].shape)
...
'''
# -----------------------Cubic Bspline-----------------------
class PixelMorph_Temp_Net(nn.Module):
    def __init__(self, config):
        super(PixelMorph_Temp_Net, self).__init__()
        ...
        self.unet = unet.UNet(in_channels = self.n, out_channels = self.ndim*self.n, dim = self.ndim, depth = depth, initial_channels = initial_channels, normalization = normalization)
        ...
        self.output_size = tuple([int(math.ceil((imsz) / c)) 
                                  for imsz, c in zip(img_size, cps)])
        self.bspl_length = int(self.output_size[0])
        self.full_length = int(self.bspl_length / self.scale)
        self.out_layer = convNd(self.dim, 1, self.dim)        
        self.transform = transformation.BSplineFFDTransformer(ndim=2, img_size=img_size, cps=cps)
        self.warp = SpatialTransformer([128,128])
        self.batch_warp = batch_SpatialTransformer([128,128])
        # 用于计算SSC特征描述
        self.r = config.r
    ...
    def calculate_ssc(self, img):
        ...
        mind = MIND.compute_ssc_batch(img, self.r, self.sigma)
    ...
    def forward(self, src, tar, mode):
        ...
        #print('src: ',src.shape,'tar: ',tar.shape)
        src_feats = self.extract(src)
        tar_feats = self.extract(tar)
        #print('src_feats: ',src_feats.shape,'tar_feats: ',tar_feats.shape)
        src_ssc = self.calculate_ssc(src_feats).to('cuda')
        tar_ssc = self.calculate_ssc(tar_feats).to('cuda')
        #print('src_ssc: ',src_ssc.shape,'tar_ssc: ',tar_ssc.shape)        
        if mode == 'train':
            src_ssc = self.feats_squeeze1(src_ssc.view(self.batch_size, 4*self.feats_channels, shape*shape))
            tar_ssc = self.feats_squeeze1(tar_ssc.view(self.batch_size, 4*self.feats_channels, shape*shape))
            #print('squeezed src_ssc: ',src_ssc.shape,'squeezed tar_ssc: ',tar_ssc.shape)
            src_ssc = src_ssc.view(self.batch_size, 4, shape, shape)
            tar_ssc = tar_ssc.view(self.batch_size, 4, shape, shape)
        else: 
            src_ssc = self.feats_squeeze1(src_ssc.view(1, 4*self.feats_channels, shape*shape))
            tar_ssc = self.feats_squeeze1(tar_ssc.view(1, 4*self.feats_channels, shape*shape))
            #print('squeezed src_ssc: ',src_ssc.shape,'squeezed tar_ssc: ',tar_ssc.shape)
            src_ssc = src_ssc.view(1, 4, shape, shape)
            tar_ssc = tar_ssc.view(1, 4, shape, shape)            
        src_ssc = self.feats_squeeze2(src_ssc)
        tar_ssc = self.feats_squeeze2(tar_ssc)        
        '''
        ...
        if self.scale < 1: 
            ...
            disp_t2i = scaled_disp_t2i
        if mode == 'train':
            #print('scaled disp shape', scaled_disp_t2i.shape)
            scaled_disp_t2i = scaled_disp_t2i.reshape(self.n, 2, self.ndim, self.full_length, self.full_length)
            disp_t2i = disp_t2i.reshape(self.n, 2, self.ndim, self.full_length, self.full_length)
            
            #flow_bspl = self.transform(disp_t2i, self.n)
            flow = disp_t2i
            batch_warped = self.batch_warp(input_image, flow)
            batch_template = torch.mean(batch_warped, 1, keepdim=True)
            scaled_template = torch.zeros([self.n, 1, 128, 128], dtype=torch.float32)
            for i in range(self.n):
                if self.scale < 1:
                    scaled_template[i,:,:,:] = F.interpolate(batch_template[i,:,:,:,:], 
                                                        size = self.img_size, 
                                                        mode = 'bilinear' if self.dim == 2 else 'trilinear', align_corners = True)
                else:
                    scaled_template = batch_template

            res = {'disp_t2i':flow, 
                'scaled_disp_t2i':scaled_disp_t2i,  
                'warped':batch_warped, 
                'template':batch_template, 
                'scaled_template':scaled_template}
        else:
            scaled_disp_t2i = scaled_disp_t2i.reshape(1, 2, self.ndim, self.bspl_length, self.bspl_length)
            disp_t2i = disp_t2i.reshape(1, 2, self.ndim, self.full_length, self.full_length)
            #flow_bspl = self.transform(disp_t2i[0], 1)
            flow = disp_t2i[0]
            warped = self.warp(input_image.permute(1,0,2,3), flow)
            template = torch.mean(warped, 0, keepdim=True)
            if self.scale < 1:
                scaled_template = F.interpolate(template, 
                                                    size = self.img_size, 
                                                    mode = 'bilinear' if self.dim == 2 else 'trilinear', align_corners = True)
            else:
                scaled_template = template
            #print('disp_t2i', flow.shape,'scaled_disp_t2i',scaled_disp_t2i.shape, 'warped', warped.shape,'template',template.shape,'scaled_template', scaled_template.shape)
            res = {'disp_t2i':flow, 
                'scaled_disp_t2i':scaled_disp_t2i[0],  
                'warped':warped, 
                'template':template, 
                'scaled_template':scaled_template}
        return res
