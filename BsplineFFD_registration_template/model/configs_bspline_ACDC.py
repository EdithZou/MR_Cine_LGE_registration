...
def get_Bspl_config():
    ...
    config.r = 0
    config.sigma = 0.5    
    config.pair_disp_indexes = [0, 1]# 选择从m2f的帧
    config.ndim = 2
    config.n = 2 # input images number, for pairwise registration is 2
    config.batch_size = 2    
    config.img_size = (128, 128)
    config.scale = 0.25
    config.depth = 5
    config.initial_channels = 128
    config.normalization = True
    config.cps = (4, 4)
    config.enc_channels = (16, 32, 32, 32, 32)
