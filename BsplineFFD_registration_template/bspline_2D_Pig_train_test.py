...
def train():
    ...
    dataset_valid_img = torch.zeros([2, 20, 256, 256], dtype=torch.float32)
    dataset_valid_label = torch.zeros([2, 20, 256, 256], dtype=torch.int8)
    ...
    for _form in range(2):
        ...
        for _num in range(len(train_list)):
            ...
            # 读取对应的标签文件
            label_pattern = os.path.join(args.train_dir, f"EP{subject}_{subject_forms[_form]}_mask.nii.gz")
    ...
    for _iter in range(1, args.n_iter + 1):
        ...
        for _step in range(steps):
            ...
            label_fixed = torch.zeros([args.batch_size, 1, 256, 256], dtype=torch.float32)
            label_moving = torch.zeros([args.batch_size, 1, 256, 256], dtype=torch.float32)
            ...
            for i in range(args.batch_size):
                ...
                sim_loss += sim_loss_fn(res['warped'][i], res['template'][i])
        ...
        with torch.no_grad():
            ...
            for _num in range(len(valid_list)):
                ...
                dice_loss = dice_fn(label_fixed, label_warped, labelorders=[1,2])
        ...
        if loss_valid <= best_valid_loss:
            ...
            save_file_dir = os.path.join(args.model_dir, "pig_bspline_train.pth")
...
def test():
    ...
    model = bspl.CubicBSplineNet(config).cuda()
    stn = SpatialTransformer([256,256]).cuda()
    stn_img = SpatialTransformer([256,256], mode='bilinear').cuda()
    dice_fn = Dice().cuda()
    calcdisp = CalcDisp(config)
    ...
    # 测试序列
    test_list = np.arange(80, 105)

    # 读取图像数据
    dataset_img = torch.zeros([2, 25, 256, 256], dtype=torch.float32)
    dataset_label = torch.zeros([2, 25, 256, 256], dtype=torch.int8)
    subject_forms = ["CINE", "DE"]
    ...
    for _form in range(2):
        ...
        for _num in range(len(test_list)):
            ...
            if len(file_list) > 0:
                file_path = file_list[0]
                data = torch.from_numpy(nib.load(file_path).get_fdata()).type(torch.float32).squeeze()
                data = torch.squeeze(data)
                dataset_img[_form, _num, :, :] = (data - data.min()) / (data.max() - data.min())
            # 读取对应的标签文件
            label_pattern = os.path.join(args.test_dir, f"EP{subject}_{subject_forms[_form]}_mask.nii.gz")
            label_list = glob.glob(label_pattern)
            if len(label_list) > 0:
                file_path_label = label_list[0]
                data_label = torch.from_numpy(nib.load(file_path_label).get_fdata()).type(torch.int8).squeeze()
                data_label = torch.squeeze(data_label)
    ...
    for _num in range(len(test_list)):
        ...
        label_moving[0, 0, :, :] = dataset_label[0, _num, :, :]
        label_fixed[0, 0, :, :] = dataset_label[1, _num, :, :]
        img_fixed = img_fixed.cuda()
        img_moving = img_moving.cuda()
        label_fixed = label_fixed.cuda()
        label_moving = label_moving.cuda()
        ...
        # 计算dice
        if args.dice:
            dice_list.append([dice_fn(label_fixed, label_moving, labelorders=[1,2]).item(), dice_fn(label_fixed, label_warped, labelorders=[1,2]).item()])
        # 计算psnr
        if args.psnr:
